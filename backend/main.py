...
async def chat(request: Request):
    ...
    if intent == "PROPERTY":
        ...
        try:
            ...
            # ✅ UPDATE (minimal): pass through properties_full if handler returned it
            response = ChatResponse(
                reply=result["reply"],
                ...
                page=1,
                per_page=10,
                filters_used=result.get("filters") or result.get("filters_used") or {},
                properties_full=result.get("properties_full")  # ✅ NEW
            )
